...
const ProfileImageModal: FC<ProfileModalState> = ({ isProfileImageModal, setIsProfileImageModal }) => {
  ...
  const attachProfileImage = async (sessionData: SessionData, profileImagePath: string) => {
    await axios.put(`${process.env.API_ROOT_URL}/api/v1/users/${sessionData.userId}/profile_images/attach`, profileImagePath, {
      headers: {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      if (dataConfirmAlert('アップロードした画像を登録しますか？')) {
        const uploadUrl = await getUploadUrl({ userId: sessionData.userId, filename: file.name, byteSize: file.size, contentType: file.type });
    ...
    <div onClick={handleClickOutside}>
      <Modal open={isProfileImageModal}>
        <Box ref={insideRef} className='absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] bg-white w-[720px] h-auto max-h-[80vh] p-10 flex flex-col overflow-y-scroll'>
          ...
          <button onClick={setDefaultProfileImage} className='w-full bg-gray-50 border border-gray-300 rounded-md py-2 text-sm text-gray-400 mb-2 hover:bg-gray-100'>デフォルト画像を設定</button>
          <div {...getRootProps()} className='bg-blue-50 border border-blue-200 rounded-md p-4 h-[140px] flex justify-center items-center cursor-pointer'>
            <input {...getInputProps()} />
            <p className='text-sm text-gray-400'>ここにファイルをドロップするか、クリックしてファイルを選択してください。</p>
          </div>
        </Box>
      </Modal>
    </div>
