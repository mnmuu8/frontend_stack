...
import { FormDataParams, onSubmitType, FormTypeProps, setFormTeamProps } from '@/types/types';
import AppContext from '@/context/AppContext';
import StackInspectionFormTeam from './StackInspectionFormTeam';
import StackIntrospectionShowTeam from './StackIntrospectionShowTeam';
import StackFormTeam from './StackFormTeam';
import UserFormTeam from './UserFormTeam';
...
const FormModal: FC = () => {
  ...
  const setFormTeam = ({formType, setValue, control}: FormTypeProps): setFormTeamProps | undefined  => {
    if (formType === 'createStack') {
      return {
        label: '積み上げを作成',
        component: <StackFormTeam setValue={setValue} control={control} />,
    ...
    if (formType === 'createStackIntrospection') {
      return {
        label: '積み上げの反省を作成',
        component: <StackInspectionFormTeam control={control} />,
        button: <Button onClick={handleSubmit(onSubmit)} className='bg-blue-400 hover:bg-blue-300 text-white mx-2 w-full' type='submit'>作成</Button>
      }
    }
    ...
    if (formType === 'updateUser') {
      return {
        label: 'update User',
        component: <UserFormTeam setValue={setValue} control={control} />,
        button: <Button onClick={handleSubmit(onSubmit)} className='bg-blue-400 hover:bg-blue-300 text-white mx-2 w-full' type='submit'>更新</Button>
      }
    }
    ...
    if (formType === 'showStackIntrospection') {
      return {
        label: '積み上げの反省',
        component: <StackIntrospectionShowTeam />,
  ...
  const currentFormTeam = setFormTeam({ formType, setValue, control });
      ...
      <Modal open={formOpen}>
        <Box className="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] bg-white w-[720px] h-[80vh] p-10 flex flex-col overflow-y-scroll">
          <div className='flex-1'>
            <div className='text-center text-2xl font-bold'>{currentFormTeam?.label}</div>
            <div className='flex flex-col'>
              {currentFormTeam?.component}
            </div>
          </div>
          <div className='flex justify-center pt-6'>
            {cancelButton}
            {currentFormTeam?.button}
          </div>
        </Box>
      </Modal>
    </>
